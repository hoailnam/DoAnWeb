...
function signUp($user)
{
    ...
    $stmt = connectDatabase()->prepare("INSERT INTO user (user_name, user_pass, user_phone, user_address) 
    VALUES (:user_name,:user_pass,:user_phone,:user_address)");
    ...
    header("Location: ../index.php?Message=Welcome to our website !!!");
}
...
if (isset($_POST['user'])) {
    switch ($_POST['user']) {
        ...
        case 'signup':
            ...
            $user = new User($id, $user_name, $pass, $address, $phonenumber);

            if ($confirm == $pass) {
                if (checkEmail($user->get_username()) == 0) {
                    signUp($user);
                } else {
                    header("Location: ../signup.php?Message=Email already exist!!! Please try again");
                }
            } else {
                header("Location: ../signup.php?Message=Password incorrect! Please try again");
            }
            break;
        ...
        case 'changepass':
            ...
            if ($confirm == $new_pass) {
                if (checkPass($old_pass, $name))
                    ...
                    header("Location: ../information.php?Message=Incorrect password !!!");
            }
